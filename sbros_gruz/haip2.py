...
class Scanner:
    ...
    def scan_once(self):
        """
        Возвращает цель полёта по карте ТОЛЬКО если найден цвет:
        ...
        # ===== ВАЖНО: если цвет НЕ найден — метку НЕ ищем =====
        if best_color is None:
            msg = "no color scanned -> skip nearest aruco search"
            rospy.logwarn(msg)
            ...
            self.lines_total.append(msg)
            self.lines_total.append("")
            self._write_report_overwrite()

            return None

        # Если цвет найден — LED + sbros()
        ...
        # ===== И ТОЛЬКО ТЕПЕРЬ ищем ближайшую метку =====
...
class Flight:
    ...
    def fly_to_marker_from_scan(self, z, spd):
        ...
        if target is None:
            rospy.logwarn("scan: no color OR no marker target from FILE MAP")
            return False

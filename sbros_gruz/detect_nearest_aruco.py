    ...
    def _load_map(self):
        ...
        with open(self.map_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                # разделители: таб/пробел/запятая — неважно
                s = s.replace(",", " ")
                parts = [p for p in s.split() if p]

                # Нам нужно минимум 5 колонок: id, length, x, y, z
                if len(parts) < 5:
                    bad += 1
                    continue
...
class Scanner:
    def __init__(self, led: LedController, report_path: str, topic_name: str, image_topic: str,
                 camera_topic: str, aruco_map: ArucoMap):

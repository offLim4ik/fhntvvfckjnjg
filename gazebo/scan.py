...
# Универсальный путь к домашней папке текущего пользователя
HOME_DIR = os.path.expanduser("~")
BASE_DIR = os.path.join(HOME_DIR, FOLDER_NAME)
...
class Scanner:
    def __init__(self, led: LedController, report_path: str, topic_name: str, image_topic: str,
        ...
        # !!! FIX: создать папку перед записью отчета
        ...
        rospy.loginfo("Report path: %s", self.report_path)
    ...
    def _image_cb(self, msg):
        ...
        # публикуем аннотированную картинку постоянно
        _, _, vis = self._scan_frame(img, draw=True)
        if vis is not None:
            try:
...
class Flight:
    ...
    def run(self):
        ...
        # Поиск 1
        self.popolam1()
        self.navigate_wait(x=0.5, y=0, z=z, speed=spd, frame_id="aruco_map")
